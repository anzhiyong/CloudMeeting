...
{
    qDebug() << "ChatMessage::setText 接收到的文本: 长度=" << text.length() << " 内容=" << text;
    m_msg = text;
    m_userType = userType;
    m_time = time;
...
{
    qDebug() << "ChatMessage::fontRect 接收到的文本: 长度=" << str.length() << " 内容=" << str;
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
    int iconSpaceW = 20;
    ...
    // 计算文本实际宽度（不包含m_spaceWid）
    int actualTextWidth = size.width() - m_spaceWid;
    // 设置最小宽度，确保能够显示文本
    int minTextWidth = 50; // 最小文本宽度，避免太短导致文本被截断
    actualTextWidth = qMax(actualTextWidth, minTextWidth);
    
    if(size.width() < (m_textWidth+m_spaceWid)) {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), m_lineHeight/4*3 + 10, actualTextWidth+2*textSpaceRect, hei-m_lineHeight);
        m_kuangRightRect.setRect(this->width() - actualTextWidth - 2*textSpaceRect - iconWH - iconSpaceW - iconRectW - sanJiaoW,
                                 m_lineHeight/4*3 + 10, actualTextWidth+2*textSpaceRect, hei-m_lineHeight);
    } else {
    ...
    qDebug() << "m_textLeftRect: x=" << m_textLeftRect.x() << " y=" << m_textLeftRect.y() << " width=" << m_textLeftRect.width() << " height=" << m_textLeftRect.height();
...
//        painter.drawRoundedRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
        ...
        qDebug() << "ChatMessage::paintEvent(User_She) 绘制文本: 长度=" << m_msg.length() << " 内容=" << m_msg << " 绘制区域=" << m_textLeftRect;
...
//        painter.drawRoundedRect(m_iconRightRect,m_iconRightRect.width(),m_iconRightRect.height());
        ...
        option.setWrapMode(QTextOption::WrapAtWordBoundaryOrAnywhere);
        painter.setFont(this->font());
        qDebug() << "ChatMessage::paintEvent(User_Me) 绘制文本: 长度=" << m_msg.length() << " 内容=" << m_msg << " 绘制区域=" << m_textRightRect;
