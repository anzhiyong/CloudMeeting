...
//        qDebug() << "取出队列:" << QThread::currentThreadId();
        ...
        else
        {
            ...
            else if(text.type == TEXT_SEND)
            {
                ...
                QByteArray utf8Bytes = text.str.toUtf8();
                QByteArray data = qCompress(utf8Bytes); //压缩，使用UTF-8编码
                send->len = data.size();
                qDebug() << "TEXT_SEND: 原始长度=" << text.str.length() << " UTF8字节数=" << utf8Bytes.size() << " 压缩后长度=" << data.size();
