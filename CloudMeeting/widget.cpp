...
//    qDebug() << QThread::currentThreadId() << this;
    ...
    if(frame.isValid() && frame.isReadable())
    {
        ...
        // 只要房间里有其他人（排除自己），就发送视频
        // partner.size() > 1 表示至少有2个人（包括自己），即至少有1个其他人
        if(partner.size() > 1)
        {
            emit pushImg(img);
        }
...
Widget::~Widget()
{
    ...
    if (_ainputThread) {
        ...
        // AudioInput 对象在线程中，需要先停止
        if (_ainput) {
            _ainput->stopCollect();
            _ainput->moveToThread(nullptr); // 从线程中移除
        }
...
void Widget::on_connServer_clicked()
{
    ...
    // IP 和端口正则验证
    QRegExp ipreg(R"(((2[0-4][0-9]|25[0-5]|1\d\d|[1-9]?\d)\.){3}(2[0-4][0-9]|25[0-5]|1\d\d|[1-9]?\d))");
    QRegExp portreg(R"(^([0-9]{1,5})$)");
    QRegExpValidator ipvalidate(ipreg), portvalidate(portreg);
    int pos = 0;

    if (ipvalidate.validate(ip, pos) != QValidator::Acceptable) {
        QMessageBox::warning(this, "Input Error", "IP 地址不合法", QMessageBox::Ok);
        return;
    }
    if (portvalidate.validate(port, pos) != QValidator::Acceptable) {
        QMessageBox::warning(this, "Input Error", "端口不合法", QMessageBox::Ok);
        return;
    }

    // 尝试连接服务器
    bool connected = _mytcpSocket->connectToServer(ip, port, QIODevice::ReadWrite);
    if (connected) {
        ...
        WRITE_LOG("Succeeded connecting to %s:%s", ip.toStdString().c_str(), port.toStdString().c_str());
        QMessageBox::information(this, "Connection success", "成功连接服务器", QMessageBox::Ok);
    } else {
        QString errStr = _mytcpSocket->errorString();
        ui->outlog->setText("连接失败: " + errStr);
        WRITE_LOG("Failed to connect %s:%s, error: %s", ip.toStdString().c_str(), port.toStdString().c_str(), errStr.toStdString().c_str());
        QMessageBox::warning(this, "Connection error", errStr, QMessageBox::Ok);
    }
}
...
void Widget::datasolve(MESG *msg)
{
    ...
    {
        QByteArray ba = QByteArray((char *)msg->data, msg->len);
        QString str = QString::fromUtf8(ba);
        qDebug() << "TEXT显示: 数据长度=" << msg->len << " 字符串长度=" << str.length() << " 内容=" << str;
    ...
    else if (msg->msg_type == PARTNER_JOIN2)
    {
        ...
        {
            if(pos + sizeof(uint32_t) > msg->len)  // 确保不会越界
            {
                qWarning() << "PARTNER_JOIN2: Buffer overflow, stopping parse";
                break;
            }
            memcpy(&ip, msg->data + pos, sizeof(uint32_t));
            pos += sizeof(uint32_t);  // 只增加一次，修复重复递增的bug
...
void Widget::on_sendmsg_clicked()
{
    ...
    qDebug() << "on_sendmsg_clicked 发送文本: 长度=" << msg.length() << " 内容=" << msg;
