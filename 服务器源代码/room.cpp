            ...
            if(FD_ISSET(i, &rset))
            {
                ...
                else if(ret == 11)
                {
                    if(head[0] == '$')
                    {
						...
						msg.targetfd = i;
						uint32_t ip_from_header;
						memcpy(&ip_from_header, head + 3, 4);
						ip_from_header = ntohl(ip_from_header);  // 转换为主机字节序
						int msglen;
                        ...
                        if(msgtype == IMG_SEND || msgtype == AUDIO_SEND || msgtype == TEXT_SEND)
                        {
                            ...
                            msg.ip = user_pool->fdToIp[i];  // 使用存储的IP（主机字节序）
...
void* accept_fd(void *arg) //accept fd from father
{
    ...
    while(1)
    {
        // 读取命令字符和客户端IP地址（5字节：1字节命令 + 4字节IP）
        char buf[5];
        int n;
        if((n = read_fd(fd, buf, 5, &tfd)) <= 0)
        {
        ...
        char c = buf[0];
        uint32_t client_ip;
        memcpy(&client_ip, buf + 1, 4);  // 已经是主机字节序
        ...
        if(c == 'C') // create
        {
            ...
            user_pool->owner = tfd;
            user_pool->fdToIp[tfd] = client_ip;  // 使用客户端发送的IP地址
            user_pool->num++;
...
//                user_pool->fds[user_pool->num++] = tfd;
                ...
                for(int i = 0; i <= maxfd; i++)
                {
                    ...
                    {
                        uint32_t ip = htonl(user_pool->fdToIp[i]);  // 转换为网络字节序发送
    ...
    */
    ...
    for(;;)
    {
        ...
        if(msg.msgType == CREATE_MEETING_RESPONSE || msg.msgType == PARTNER_JOIN2)
        {
            ...
            memcpy(sendbuf + len, &placeholder, sizeof(uint32_t));
            len += 4;
        }
        ...
        {
            uint32_t ip_network = htonl(msg.ip);  // 转换为网络字节序发送
            memcpy(sendbuf + len, &ip_network, sizeof(uint32_t));
            len+=4;
        }
