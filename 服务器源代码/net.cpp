...
uint32_t getpeerip(int fd)
{
    ...
    // 转换为主机字节序返回，统一在服务器内部使用主机字节序存储IP
...
const char *Sock_ntop(char * str, int size ,const sockaddr * sa, socklen_t salen)
{
    if(sa == NULL)
    {
        snprintf(str, size, "NULL address");
        return str;
    }
    ...
    case AF_INET:
        {
            ...
            if(inet_ntop(AF_INET, &sin->sin_addr, str, size) == NULL)
            {
                ...
                snprintf(str, size, "inet_ntop error");
                return str;
            }
    ...
    case AF_UNIX:
        {
            // AF_LOCAL 是 AF_UNIX 的别名，在 macOS 上值相同，所以只用一个 case
            struct sockaddr_un *sun = (struct sockaddr_un *) sa;
            snprintf(str, size, "Unix socket: %s", sun->sun_path);
            return str;
        }
     default:
        {
            // 打印未知地址族的详细信息用于调试
            snprintf(str, size, "unknown address family %d (salen=%d)", sa->sa_family, salen);
            err_msg("unknown address family %d in Sock_ntop", sa->sa_family);
            return str;
        }
    }
}
